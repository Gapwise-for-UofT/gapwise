...
export function validateDraft(draft: Draft, adapter: InstitutionAdapter): string[] {
  ...
  if (!adapter.parseCourseCode(draft.courseCode))
    errors.push("Enter a Carleton course code, such as BUSI 1004.");
  if (!draft.nativeSection.trim()) errors.push("Enter the native section identifier.");
  if (!draft.nativeComponentType.trim()) errors.push("Enter the component type shown by Carleton.");
  if (!draft.termLabel.trim()) errors.push("Enter the term or session shown by Carleton.");
...
export function resolveLocation(
  ...
  // Building codes may be joined to room numbers in calendar exports (TB208).
  // Names still need an exact match or a separator; ambiguity fails closed.
