...
export function parseMcMasterText(
  rawText: string,
  campus: CampusSnapshot,
  ...
  // Strategy 1: Tabular single-line format (e.g. Mosaic Class Schedule)
  for (const line of lines) {
    const courseMatches = [...line.matchAll(/\b([A-Z]{3,8})\s*(\d[A-Z0-9]{2,3})\b/gi)];
    ...
    if (timeMatch) {
      ...
      if (times) {
        ...
        if (days.length === 0) days = ["MO", "WE", "TH"];
        ...
        const secMatch = betweenCourseAndTime.match(/\b([A-Z0-9]{2,4})\b/i);
        const section = secMatch ? secMatch[1]!.toUpperCase() : "C01";

        const compMatch = line.match(/\b(LEC|TUT|LAB|PRA|SEM|CLN)\b/i);
  ...
  if (meetings.length === 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 1. Course code
      const courseMatch = line.match(/\b([A-Z]{3,8})\s*(\d[A-Z0-9]{2,3})\b/i);
      ...
      const secMatch = line.match(/\b(?:SEC|SECTION|SECT)?\s*([A-Z0-9]{2,4})\b/i);
      if (
        secMatch &&
        !["LEC", "TUT", "LAB", "PRA", "SEM", "CLN"].includes(secMatch[1]!.toUpperCase())
      ) {
          ...
          for (let j = 1; j <= 2; j++) {
            if (i + j < lines.length) {
              ...
              if (
                ...
                !candidate.match(/\b([A-Z]{3,8})\s*(\d[A-Z0-9]{2,3})\b/i)
              ) {
  ...
  if (meetings.length === 0) {
    warnings.push(
      "No class meetings could be recognized from the text. Make sure to copy your timetable from Mosaic Class Schedule.",
    );
  }
