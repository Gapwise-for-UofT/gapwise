...
export function parseQueensText(
  rawText: string,
  ...
  // Strategy 1: Tabular single-line format (SOLUS View My Classes)
  for (const line of lines) {
    const courseMatches = [...line.matchAll(/\b([A-Z]{3,4})\s*(\d{3,4}[A-Z]?)\b/gi)];
    ...
    if (timeMatch) {
      ...
      if (times) {
        ...
        const secMatch = betweenCourseAndTime.match(/\b(\d{2,3}|00[1-9]|[A-Z]\d?)\b/);
        const section = secMatch ? secMatch[1]! : "001";
  ...
  if (meetings.length === 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 1. Course code
      const courseMatch = line.match(/\b([A-Z]{3,4})\s*(\d{3,4}[A-Z]?)\b/i);
