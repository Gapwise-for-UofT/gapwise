...
function parseSummary(summary: string): {
  ...
  // Match e.g. "CPS 109 01 LEC - Computer Science I", "MTH 140", "ECN 104"
  const courseMatch = cleaned.match(/\b([A-Z]{2,4})\s*(\d{3,4})([A-Z0-9]*)\b/i);
  ...
  if (!section) {
    ...
    const secMatch = remainder.match(/^\s*(?:(?:SEC|SECTION|SECT)\s+)?([A-Z0-9]{1,3})\b/i);
    if (
      secMatch &&
      secMatch[1] &&
  ...
  if (parts.length > 1) {
    ...
    courseName = cleaned
      ...
      .replace(/^\s*(?:(?:SEC|SECTION|SECT)\s+)?[A-Z0-9]{1,3}\b/i, "")
