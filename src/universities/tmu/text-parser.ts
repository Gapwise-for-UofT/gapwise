...
function extractLocation(str: string, campus: CampusSnapshot): string | null {
  ...
  for (const b of campus.buildings) {
    for (const code of b.nativeCodes) {
      // Don't match "Mon" as code "MON" if there's no room
  ...
  for (const b of campus.buildings) {
    for (const code of b.nativeCodes) {
      if (code.length >= 3 && clean.includes(code)) {
        return code;
      }
    }
  }
...
export function parseTmuText(
  ...
  // Strategy 1: Tabular single-line format (e.g. MyServiceHub / VSB table)
  for (const line of lines) {
    const courseMatches = [...line.matchAll(/\b([A-Z]{2,4})\s*(\d{3,4})\b/gi)];
    ...
    if (timeMatch) {
      ...
      if (times) {
        ...
        const secMatch = betweenCourseAndTime.match(/\b(\d{2,3}|[A-Z]\d?)\b/);
        const section = secMatch ? secMatch[1]! : "01";
  ...
  if (meetings.length === 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 1. Course code
      const courseMatch = line.match(/\b([A-Z]{2,4})\s*(\d{3,4})\b/i);
      if (courseMatch && !IGNORED_WORDS.has(courseMatch[1]!.toUpperCase())) {
        currentCourseCode = `${courseMatch[1]!.toUpperCase()} ${courseMatch[2]!}`;
  ...
  if (meetings.length === 0) {
    warnings.push("No class meetings could be recognized from the text. Make sure to copy your timetable from MyServiceHub (RAMSS).");
  }
