...
function parseSummary(summary: string): {
  ...
  // York codes e.g. "EECS 1022 A LEC - Programming", "MATH 1013", "PSYC 1010"
  const courseMatch = cleaned.match(/\b([A-Z]{2,4})\s*(\d{4})([A-Z0-9]*)\b/i);
  const courseCode = courseMatch
    ? `${courseMatch[1]!.toUpperCase()} ${courseMatch[2]!}`
    : cleaned || "UNKNOWN";

  const compMatch = cleaned.match(/\b(LEC|TUT|LAB|SEM|PRA|DIR)\b/i);
  ...
  if (!section) {
    ...
    if (
      secMatch &&
      secMatch[1] &&
      !["LEC", "TUT", "LAB", "SEM", "PRA", "DIR"].includes(secMatch[1].toUpperCase())
    ) {
  ...
  if (dashIndex !== -1) {
    ...
    courseName = cleaned
      ...
      .replace(/^\s*(?:\(?\b(?:LEC|TUT|LAB|SEM|PRA|DIR)\b\)?)\s*/i, "")
      .trim();
  }

  return {
    courseCode,
    nativeSection: section || "A",
    nativeComponentType,
